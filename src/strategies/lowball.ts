...
async function tryEnterSymbol(symbol: string, nextBoundary: Date, lb: LowballConfig) {
  ...
  for (const [sideLabel, tokenId] of sides) {
    for (const level of lb.buyLevels) {
      placements.push(doBuyOrder(tokenId, level, orderSizeUsd, sideLabel, symbol, expiration));
    }
  }
...
async function doBuyOrder(
  ...
  try {
    ...
    const tracked: TrackedBuy = {
      orderId, tokenId, buyPrice, placedAt: Date.now(), sideLabel, symbol, expiration,
    };
...
async function monitorBuyFill(tracked: TrackedBuy): Promise<void> {
  const { orderId, tokenId, buyPrice, sideLabel, symbol, expiration } = tracked;
  ...
  while (true) {
    ...
    try {
      ...
      if (isFilled && sizeMatched > 0) {
        ...
        await doSellOrder(tokenId, sizeMatched, buyPrice, sideLabel, symbol, lb);
        break;
      }
    } catch (e: unknown) {
...
async function doSellOrder(
  ...
  try {
    ...
    void monitorSellFill(orderId, sideLabel, sellShares, sellPrice, symbol);
  } catch (e: unknown) {
